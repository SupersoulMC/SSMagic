...
public class IceWitch extends MobM {
	...
	public IceWitch(MythicMob mythicMob, Entity entity) {
									...
									public void run() {
										for (int y = 1; y <= 4; y++) {
											...
											for (Player player : Bukkit.getOnlinePlayers()) {
												...
												if (player.getWorld().getNearbyEntities(l, 0.5, 0.5, 0.5).contains(player)
														&& !damaged.contains(player)) {
													damaged.add(player);
													player.damage(9);
												}
											}
		...
		this.getPhasedAttacks().get(0).add(new AttackM(this, 40) {
			@Override
			public void attack() {
				...
				new BukkitRunnable() {
					@Override
					public void run() {
						IceWitch.this.say("Run up to the sides or to the back!! Elsa's dropping an Ice Bomb! Be careful!!!");
						new BukkitRunnable() {
							@Override
							public void run() {
